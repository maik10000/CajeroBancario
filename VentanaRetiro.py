...
class VentanaRetiro(tk.Toplevel):
    ...
    def __init__(self, *args, numero_cuenta="",actualizar=None, **kwargs):
    ...
    def componentes(self):
        ...
        label = tk.Label(self, text="Otra cantidad:", font=self.font_style, bg=color.BLANCO)
        ...
        ttk.Style().map('btn_ret.TButton', background=[('pressed', color.VERDE_30), ('active', color.VERDE_00)])
        ttk.Style().map('btn_r2.TButton', background=[('pressed', color.VERDE_30), ('active', color.AZUL_57)])
        ...
        btn_monto2 = ttk.Button(conta, text='$10', style='btn_ret.TButton', command=lambda: self.retirar_cantidades(10))
        btn_monto2.grid(column=1, row=0, pady=50)

        btn_monto3 = ttk.Button(conta, text='$20', style='btn_ret.TButton', command=lambda: self.retirar_cantidades(20))
        btn_monto3.grid(column=0, row=1)
        btn_monto4 = ttk.Button(conta, text='$50', style='btn_ret.TButton', command=lambda: self.retirar_cantidades(50))
        btn_monto4.grid(column=1, row=1)

        btn_monto5 = ttk.Button(conta, text='$100', style='btn_ret.TButton', command=lambda: self.retirar_cantidades(100))
        btn_monto5.grid(column=0, row=2)
        btn_monto6 = ttk.Button(conta, text='$150', style='btn_ret.TButton', command=lambda: self.retirar_cantidades(150))
        btn_monto6.grid(column=1, row=2, pady=50)

        boton_aceptar = ttk.Button(self, text="Aceptar", command=self.retirar, style='btn_ret.TButton')
        boton_aceptar.place(x=230, y=603, width=150, height=40)
        boton_cancelar = ttk.Button(self, text="Cancelar", style='btn_r2.TButton', command=self.cancelar)
            ...
            try:
                ...
                if c % 5 == 0:
                    if 10< c < 15000:
                        ...
                        self.mensaje('El monto tiene que ser de 0.5 hasta 15.000', clor=color.AMARILLO_2F)
                else:
                    self.mensaje('Sistema solo puede procesar cantidades multiplos de 5', clor=color.AMARILLO_2F)
